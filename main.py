...
def get_calendar_events(service, timezone: str, calendar_id: str):
    ...
    start = datetime.datetime.combine(
        start_date, datetime.time(0), pytz.timezone(timezone)
    ).astimezone(pytz.UTC)
    ...
    for event in events['items']:
        ...
        if (
            ...
            trigger_times.append(
                ...
                - datetime.timedelta(seconds=trigger_offset_seconds)
            )
...
def main():
    ...
    trigger_times = calculate_trigger_times(events, timezone, end_threshold_seconds, trigger_offset_seconds)
